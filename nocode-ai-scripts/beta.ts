...
const model = "nocode-ai/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/nocode-ai")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with nocode-ai...`)
  ...
  try {
    await $`nocode-ai run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  nocode-ai failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with nocode-ai")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with nocode-ai...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/nocode-ai`.",

...
const EXTENSION_NAME = "nocode-ai"
...
async function main() {
  ...
  // Delete any existing branches for nocode-ai updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating NocodeAi extension to v${cleanVersion}"`

...
import { createOpencode } from "@nocode-ai-ai/sdk"
...
`)
  ...
  const nocode-ai = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await nocode-ai.client.session.create()
    const result = await nocode-ai.client.session
      .prompt({

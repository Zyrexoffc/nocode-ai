...
- Author package entrypoint is `@nocode-ai-ai/plugin/tui`.
...
```json
{
  "$schema": "https://nocode-ai.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/nocode-ai-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@nocode-ai-ai/plugin/tui`.
- `@nocode-ai-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@nocode-ai-ai/plugin/tui"
...
- The value is a semver range checked against the running NocodeAi version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.nocode-ai` is absent, no check is performed (backward compatible).
...
- `nocode-ai plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `nocode-ai plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.nocode-ai` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.nocode-ai`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.nocode-ai`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.nocode-ai` dir or `NOCODE_AI_CONFIG_DIR`, NocodeAi installs `@nocode-ai-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@nocode-ai-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.nocode-ai/themes` area near the plugin config source.
...
- `--pure` / `NOCODE_AI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.nocode-ai/plugins/tui-smoke.tsx`
- Local vim plugin: `.nocode-ai/plugins/tui-vim.tsx`
- Local smoke config: `.nocode-ai/tui.json`
- Local smoke theme: `.nocode-ai/plugins/smoke-theme.json`

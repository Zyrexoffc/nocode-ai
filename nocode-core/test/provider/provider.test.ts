...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@nocode-ai-ai/plugin": "0.0.0" } } } }),
  )
}

function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("nocode-ai")]
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "nocode-ai" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".nocode-ai")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".nocode-ai", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("nocode-ai loader keeps paid models when config apiKey is present", async () => {
...
test("nocode-ai loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "nocode-ai.json"),

...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const nocode-aiModel = {
      ...openaiModel,
      providerID: "nocode-ai",
      api: {
        id: "nocode-ai-test",
        url: "https://api.nocode-ai.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, nocode-aiModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.nocode-ai?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.nocode-ai?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          nocode-ai: { itemId: "msg_nocode-ai" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              nocode-ai: { itemId: "msg_nocode-ai_part" },
    ...
    expect(result[0].providerOptions?.nocode-ai?.itemId).toBe("msg_nocode-ai")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.nocode-ai?.itemId).toBe("msg_nocode-ai_part")

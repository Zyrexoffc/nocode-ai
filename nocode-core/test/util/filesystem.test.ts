...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nocode-ai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nocode-ai.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nocode-ai.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nocode-ai.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nocode-ai.json", "nocode-ai.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "nocode-ai.json"),
        path.join(tmp.path, "nocode-ai.jsonc"),
        path.join(project, "nocode-ai.json"),
        path.join(project, "nocode-ai.jsonc"),
      ])
    })
  })

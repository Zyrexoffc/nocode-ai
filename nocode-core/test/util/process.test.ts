...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.NOCODE_AI_TEST ?? "")'), {
      env: {
        NOCODE_AI_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "NOCODE_AI_TEST_SHELL"], {
      shell: true,
      env: {
        NOCODE_AI_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("NOCODE_AI_TEST_SHELL=ok")
  })

...
test("plan agent denies edits except .nocode-ai/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".nocode-ai/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".nocode-ai", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.NOCODE_AI_TEST_HOME
  process.env.NOCODE_AI_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".nocode-ai", "skill", "perm-skill")

...
const { Npm } = await import("@nocode-ai-ai/core/npm")
...
describe("plugin.loader.shared", () => {
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "nocode-ai.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "nocode-ai.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["nocode-ai-openai-codex-auth@1.0.0", "nocode-ai-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("nocode-ai-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("nocode-ai-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "nocode-ai.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "nocode-ai.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "nocode-ai.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.NOCODE_AI_PURE
    process.env.NOCODE_AI_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.NOCODE_AI_PURE
      } else {
        process.env.NOCODE_AI_PURE = pure
      }
    }
  })

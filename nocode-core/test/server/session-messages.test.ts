...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.NOCODE_AI_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.NOCODE_AI_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.NOCODE_AI_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.NOCODE_AI_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}

...
const FILES = [
  "AGENTS.md",
  ...(Flag.NOCODE_AI_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
  "CONTEXT.md", // deprecated
]

function globalFiles() {
  const files = []
  if (Flag.NOCODE_AI_CONFIG_DIR) {
    files.push(path.join(Flag.NOCODE_AI_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.NOCODE_AI_DISABLE_CLAUDE_CODE_PROMPT) {
...
export class Service extends Context.Service<Service, Interface>()("@nocode-ai/Instruction") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.NOCODE_AI_DISABLE_PROJECT_CONFIG) {
          return yield* fs
        ...
        if (!Flag.NOCODE_AI_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no NOCODE_AI_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.NOCODE_AI_CONFIG_DIR, Flag.NOCODE_AI_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })

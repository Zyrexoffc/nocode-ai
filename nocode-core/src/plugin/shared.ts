...
export const DEPRECATED_PLUGIN_PACKAGES = ["nocode-ai-openai-codex-auth", "nocode-ai-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, nocode-aiVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(nocode-aiVersion) || semver.major(nocode-aiVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.nocode-ai
  if (typeof range !== "string") return
  if (!semver.satisfies(nocode-aiVersion, range)) {
    throw new Error(`Plugin requires nocode-ai ${range} but running ${nocode-aiVersion}`)
  }
}

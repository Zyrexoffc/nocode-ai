...
import { Module } from "@nocode-ai-ai/core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.NOCODE_AI_DISABLE_LSP_DOWNLOAD) return
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "nocode-ai-jdtls-data"))
    return {
      process: spawn(
        java,
        [

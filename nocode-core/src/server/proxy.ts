...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-nocode-ai-directory")
  out.delete("x-nocode-ai-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-nocode-ai-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-nocode-ai-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }

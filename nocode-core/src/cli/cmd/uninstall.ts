...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall nocode-ai and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall NocodeAi")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g nocode-ai-ai",
      pnpm: "pnpm uninstall -g nocode-ai-ai",
      bun: "bun remove -g nocode-ai-ai",
      yarn: "yarn global remove nocode-ai-ai",
      brew: "brew uninstall nocode-ai",
      choco: "choco uninstall nocode-ai",
      scoop: "scoop uninstall nocode-ai",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "nocode-ai", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using NocodeAi!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# nocode-ai") || content.includes(".nocode-ai/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".nocode-ai/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".nocode-ai/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".nocode-ai"))
    ) {
      continue
    }

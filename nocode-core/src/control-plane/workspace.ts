...
import { Slug } from "@nocode-ai-ai/core/util/slug"
...
export const create = fn(CreateInput.zod, async (input) => {
  ...
  const env = {
    NOCODE_AI_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    NOCODE_AI_WORKSPACE_ID: config.id,
    NOCODE_AI_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.NOCODE_AI_EXPERIMENTAL_WORKSPACES) return

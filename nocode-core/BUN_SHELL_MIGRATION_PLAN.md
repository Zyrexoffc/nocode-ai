...
Replace runtime Bun shell template-tag usage in `packages/nocode-ai/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/nocode-ai/src` is removed except:
